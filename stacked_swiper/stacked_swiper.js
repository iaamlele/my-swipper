        ...
        if(this.imgArr.length === 1) {
            ...
            if(this.nowIndex === this.retImgArr.length - 2) {
                ...
                setTimeout(function() {
                    ...
                    that.mainDom.style.left = `-${(that.imgWidth - that.gap) * (that.nowIndex - 1)}px`;
                    that.setScale();
                    that.setActiveSpot();  
                }, that.aniTime);
            }else {
        ...
        if(this.imgArr.length === 1) {
            ...
            if(this.nowIndex === 1) {
                ...
                setTimeout(function() {
                    that.mainDom.style.transition = 'none';
                    that.setScale();
                    that.mainDom.style.left = `${-(parseInt(that.mainDom.children[that.nowIndex].style.left) - that.diffLen - that.gap)}px`;
                }, that.aniTime);
            }else {
                this.setScale();
                this.setActiveSpot();
    ...
    setScale:function() {
        for(let i = 0; i < this.mainDom.children.length; i++) {
            if(this.imgArr.length === 2) {
                ...
                this.mainDom.children[i].style.left = `${i * (this.imgWidth + this.gap)}px`;
            }
